...
# Si ya hay datos, no volver a seedear (para Docker restart)
if db.query(User).count() > 0:
    print("ℹ️  Base de datos ya inicializada, saltando seed.")
    db.close()
    sys.exit(0)
...
tests = [
    ...
    Test(code="UROCULT2",name="Urocultivo con antibiograma",        abbreviation="Uroc2",category_id=cat_micro.id, unit="informe",        turnaround_hours=72,  price=1100.0),
...
def make_order(pat, doc, urgency, days_ago, test_codes, results_values=None):
    ...
    prefix = order_date.strftime("%d%m%y")
...
print(f"   {db.query(User).count()} usuarios | {db.query(Doctor).count()} médicos")
print(f"   {db.query(TestCategory).count()} categorías | {db.query(Test).count()} análisis")
print(f"   {db.query(Equipment).count()} equipos | {db.query(Patient).count()} pacientes")
print(f"   {db.query(Order).count()} órdenes | {db.query(Result).count()} resultados")
